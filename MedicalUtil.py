...
def Jaccard(result, reference):
    jc = metric.binary.jc(result, reference)
    return jc
...
def OR(result, reference):
    ...
    return float(size_i1 - intersection)/float(size_i1+size_i2-intersection)
...
def avd(result, reference, voxelspacing=None, connectivity=1):
    hd1 = np.mean(__surface_distances(result, reference, voxelspacing, connectivity))
    hd2 = np.mean(__surface_distances(reference, result, voxelspacing, connectivity))
    hd = max(hd1, hd2)
    return hd
...
def assd(result, reference, voxelspacing=None, connectivity=7):
    """
...
def asd(result, reference, voxelspacing=None, connectivity=7):
    """
...
def obj_assd(result, reference, voxelspacing=None, connectivity=7):
    """
    Average symmetric surface distance.
...
def obj_asd(result, reference, voxelspacing=None, connectivity=7):
    """
